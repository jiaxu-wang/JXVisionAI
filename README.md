...
- ✅ **Redis** 存储流配置与检测记录（保留天数可配置）  
- ✅ Web：**状态概览**（检测能力计数含打电话/玩手机等扩展项、在线/总流、今日告警统计）、**检测配置**（原视频流管理）、**历史告警**（时间范围/视频流/检测类型筛选、分页条数 5～50）、**流预览**（服务端 RTSP→MJPEG，浏览器中查看）  
- ✅ 登录鉴权（密钥）  
- ✅ 日志含流名称标识  
...
│   │   ├── preview_cache.py       # 预览 MJPEG 与带框缓存
...
| `AUTO_REFRESH_INTERVAL` | 管理端历史页自动刷新间隔（秒） | 20 |
...
- 在 **「检测配置」** 中添加/编辑视频流、每路 **检测类型**（弹窗内开关）、保存后需 **重启进程** 使检测线程加载新配置。  
...
| `GET /api/preview?stream_id=` | RTSP→MJPEG 预览（`stream_id` 须为当前配置中的流） |
