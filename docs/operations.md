...
- GPU 推理 ONNX 需 `onnxruntime-gpu` 且 `[basic]` `inference_device = gpu`；日志中 provider 应为 `CUDAExecutionProvider`（否则检查 CUDA 与依赖）
...
- [x] Web 训练实验室（行业级：审核标注、独立测试集、部署门禁、yolo26s、类别契约）
