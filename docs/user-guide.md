...
5. 「刷新在线状态」显示 SIP **在线** 后，在**通道**里点 **预览**（仅点播，画面旁可云台）或 **接入分析**（写入检测配置）
...
在 **设备接入** 对应连接上点击 **预览**（国标在通道配置内）。仅使用 **ZLM WebRTC**（低延迟，源有兼容音轨时可听声）。状态概览只看在线，不提供预览。标题栏 **截图保存** 会把当前画面下载到本机（JPEG，文件名含通道名与时间）。
