...
| 页面 | 功能 |
|------|------|
| **状态概览** | 各流在线/离线、今日告警数、运行指标、ZLM 状态（预览请到设备接入） |
...
5. 「刷新在线状态」显示 SIP **在线** 后，在**通道**里点 **预览**（仅点播）或 **接入分析**（写入检测配置）
...
在 **设备接入** 对应连接上点击 **预览**（国标在通道配置内）。仅使用 **ZLM WebRTC**（低延迟，源有兼容音轨时可听声）。状态概览只看在线，不提供预览。

前置：`[zlm] enabled=true`、secret 正确、compose 映射 **UDP/TCP 8000**（宿主机 **18000**）；跨机将 `public_host` / `rtc.externIP` 设为浏览器可达 IP。浏览器需点播放并可能取消静音。
