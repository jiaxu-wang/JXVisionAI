"""
S3 兼容对象存储（MinIO、AWS、阿里云 OSS 等）。

告警截图默认前缀 ``visionai/snapshots/``，人脸库照片前缀 ``visionai/face_library/``。
前缀由配置 ``s3_path_prefix`` / ``REDIS_KEY_PREFIX`` 同类历史标识控制，与包名无关。
"""
...
class S3CompatibleStorage:
    ...
    def _apply_lifecycle(self) -> None:
        ...
        from jxvisionai.config.settings import DETECTION_RETENTION_DAYS
