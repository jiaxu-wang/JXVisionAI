...
# 保证以 ``python -m jxvisionai`` 启动时也能解析项目根下的相对导入
...
from jxvisionai.utils.alert_email import (
    normalize_stream_alert_emails,
    normalize_stream_alert_email_enabled,
    smtp_is_configured,
)
from jxvisionai.utils.alert_webhook import (
...
from jxvisionai.config.detection_catalog import catalog_items_for_api, normalize_detections
from jxvisionai.core.face_recognition_config import normalize_face_recognition_config
from jxvisionai.config.ini_manager import (
    config_meta,
    patch_config_updates,
    read_structured_units,
)
from jxvisionai.core import stream_sync
from jxvisionai.core.preview_cache import get_preview_jpeg, get_preview_jpeg_meta
from jxvisionai.core.preview_hls import (
    ffmpeg_available,
    safe_hls_basename,
    session_dir,
...
# 导入流状态管理
try:
    from jxvisionai.core.state_manager import stream_status, stream_status_lock
except:
    # 如果导入失败，初始化默认状态
    stream_status = {}
...
try:
    from jxvisionai.core.object_storage import get_object_storage
except ImportError:
    def get_object_storage():  # type: ignore
        return None

try:
    from jxvisionai.core import preview_webrtc
except ImportError:
...
def api_face_library_enroll():
    import cv2
    import numpy as np
    from jxvisionai.core import face_library
...
from jxvisionai.web.training_routes import init_training_lab  # noqa: E402
